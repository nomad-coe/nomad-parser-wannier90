![](https://github.com/nomad-coe/nomad-parser-wannier90/actions/workflows/actions.yml/badge.svg)
![](https://coveralls.io/repos/github/nomad-coe/nomad-parser-wannier90/badge.svg?branch=main)
...
# NOMAD's parser for Wannier90

This is a parser plugin for the [Wannier90](http://www.wannier.org/) input/output files.
...
1. Add the configurations related to NOMAD. This is already implemented through the plugin definition file ```src/nomad_parser_wannier90/nomad_plugin.yaml```:

    ```yaml
    plugin_type: parser
    name: parsers/wannier90
    description: |
      This plugin is used to parsed Wannier90 files into the NOMAD schema.
    ```
...
4. Add to your local NOMAD installation environment the `PYTHONPATH` to your plugin. This can be done either by running the following command every time you start a new terminal for running the appworker, or by adding it to your virtual environment in the `<path-to-local-nomad-installation>/.pyenv/bin/activate` file:
    ...
    ```sh
    export PYTHONPATH="$PYTHONPATH:<path-to-nomad-parser-wannier90-cloned-repo>/src"
    ```

If you are working in this repository, you just need to activate the environment to start working using the ```nomad-parser-wannier90``` package locally in your own Python scripts.
